...
public class StringGenerator implements SourceGenerator {
    @Override
    public @Nonnull List<?> generate(@Nonnull TripleSet tripleSet, @Nonnull File tempDir) {
        ...
        for (ImmutablePair<byte[], RDFLang> p : new ByteGenerator().generateWithLang(tripleSet)) {
            if (ByteGenerator.CAN_GUESS.contains(p.right) && !p.right.isBinary())
                list.add(new String(p.left, StandardCharsets.UTF_8));
        }
        return list;
    }
