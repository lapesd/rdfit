...
public class CommonsArrayListenerParser extends BaseJavaListenerParser {
    public CommonsArrayListenerParser() {
        super(TripleLike[].class, Triple.class, Quad.class);
    }

    @Override protected boolean feed(@Nonnull ListenerFeeder feeder, @Nonnull Object element) {
        if (element instanceof Quad && ((Quad) element).getGraphName().isPresent())
            return feeder.feedQuad(element);
        else
